...
* Spring AOP proxy - возможность отслеживать редактирование/удаление любых записей (на примере случайных имен для пользователей) используя аннотации для аудита
* HTTP WebFilter - используется для доступа к текущему пользователю через ThreadLocal в тех местах, где о нем требуется информация
